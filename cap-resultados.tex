\chapter{Resultados}\index{Experiment Results}
...
% como já discutido na Seção \ref{sec:number_of_detected_events}.
...
% - \todo{Só para te avisar: expliquei aqui como ler os diagramas de caixa.}
...
\label{fig:BoxPlot_corr_IS_7-dez-is-r}
\end{subfigure}%
%\end{figure}
%\begin{figure}
\begin{subfigure}{.5\textwidth}
...
\label{fig:BoxPlot_BusB_IS_7-dez-is-r}
\end{subfigure}%
\caption{Latências das categorias de tipos de evento por intervalos de cinco minutos ao longo da execução 2 do experimento utilizando o algoritmo de balanceamento de carga por Similaridade de Entrada.}
\label{fig:BoxPlot_IS_7-dez-is-r}
\end{figure}

A Figura \ref{fig:BoxPlot_IS_7-dez-is-r} mostra, para cada uma das categorias de eventos, a variação da latência ao longo da execução 2 do experimento com o algoritmo de balanceamento de carga por Similaridade de Entrada. 
...
%Os tipos de eventos utilizados no experimento foram escolhidos para a detecção de um número suficiente de respostas, para a obtenção de mais dados de latência de forma que ainda detectassem ocorrências importantes para a administração da frota de ônibus municipal\todo{esse comentário parece estar no lugar errado, porque ele fala sobre o desenho do experimento, não dos resultados}. 
...
%Para servir os serviços urbanos de transporte, como os do cenário experimental utilizado neste trabalho, essa latência não impacta significativamente o intervalo de tempo entre a  ocorrência de um problema nas vias de circulação urbanas e a chegada das autoridades para resolução do problema detectado\todo{mas que história é essa da chegada de autoridades??? }. 
...
%Portanto, no cenário usado no experimento, o uso de um algoritmo de balanceamento de carga não foi expressivamente mais eficaz que o outro na economia de recursos computacionais.
