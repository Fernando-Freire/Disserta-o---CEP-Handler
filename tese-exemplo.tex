...
\begin{center}
    ...
    %Ele também é parte do INCT da Internet do Futuro para Cidades Inteligentes financiado pelo CNPq proc. 465446/2014-0, Coordenação de Aperfeiçoamento de Pessoal de Nível Superior -- Brasil (CAPES) -- Código de Financiamento 001, FAPESP proc. 14/50937-1 e FAPESP proc. 15/24485-9.
...
%O número de eventos detectados variou entre uma execução e outra, o que pode ser atribuído a variação na latência. 
% A média do tempo de uso do ambiente de execução não apresentou diferença significativa entre o uso de um algoritmo de balanceamento de carga ou outro.


%Nos experimentos, o fluxo de dados entrando no sistema variou de seis a dez mil eventos por minuto. Os resultados mostraram que o sistema escala automaticamente, aumentando o número de instâncias conforme sua carga de entrada aumenta, e que ele mantém a variação na latência da detecção de eventos baixa.\\
