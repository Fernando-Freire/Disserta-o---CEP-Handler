...
Como a plataforma InterSCity oferece um transmite internamente os dados em tempo real por meio de um \textit{broker} de mensagens, o \texttt{Event Composer} se inscreve%\todo{se inscreve onde? precisar explicar que a plataforma interscity usa um broker de mensagens, onde publica os dados dos recursos em diferentes tópicos}
...
Como exemplo, considere um cenário no qual há uma instância \textit{A} quer realocar um tipo de evento \textit{T} e uma instância \textit{B} que não está em sobrecarga e que poderia, portanto, receber \textit{T}. A realocação, nesse caso, se dá da seguinte forma: 
\begin{enumerate}
...
\begin{algorithmic}[1]
\STATE indiceInvertidoDeEntrada $\leftarrow$ \{\} \COMMENT{\textit{isto é um dicionario de tipoDeEventoDeEntrada por tiposDeEventoQueOTemComoEntrada}}  
\STATE RankDoTipoDeEvento $\leftarrow$ \{\} \COMMENT{\textit{isto é um dicionário de nivelDeUsoDeTipoDeEventoDeEntrada por objetoDeBuscaPorTiposPorTamanhoDoFluxoDeEventos}}  
...
\ENDFOR
\ENDFOR
\FOR{cada tDeEvento $\in$ instância.tiposDeEventos} 
\STATE menorNivelDeUso $\leftarrow$ Inteiro.Máximo
\FOR{cada tDeEntrada $\in$ tDeEvento.tiposDeEventosDeEntrada}
\IF{indiceInvertidoDeEntrada[tDeEntrada].tamanho < menorNivelDeUso}
\STATE {menorNivelDeUso $\leftarrow$ indiceInvertidoDeEntrada[tDeEntrada].tamanho }
\ENDIF
\ENDFOR
\STATE RankDoTipoDeEvento[menorNivelDeUso].inclui(tDeEvento);
\ENDFOR
\RETURN tipoDeEventoParaRealocar $\leftarrow$ RankDoTipoDeEvento.menorRank.BuscaPorTiposPorTamanhoDoFluxoDeEventos
\end{algorithmic}
\end{algorithm}

\begin{algorithm}[tb]
\caption{BuscaTiposPorTamanhoDoFluxoDeEventos(instância)}
\label{alg:relocation_resource_usage}
\begin{algorithmic}[1]
\STATE rankDeTempo $\leftarrow$ instância.tiposDeEventosComJanelaDeTempo \COMMENT{\textit{lista de pares (tipoDeEvento, intervaloDeJanelaDeTempo)} - Esta lista é atualizada no cadastro ou remoção de tipos de eventos.}
\STATE rankDeFluxo $\leftarrow$ instância.tiposDeEventosComJanelaDeEventos \COMMENT{\textit{lista de pares (tipoDeEvento, (maiorTamanhoJanelaDeEventosDeEntrada/instância.taxaDeConsumoDeEventosDeEntrada))} - Esta lista é atualizada no cadastro ou remoção de tipos de eventos.}
\STATE tipoDeEventoTempo $\leftarrow$ rankDeTempo.menorRank
\STATE tipoDeEventoFluxo $\leftarrow$ rankDeFluxo.menorRank
\IF{tipoDeEventoTempo.menorRank <= tipoDeEventoFluxo.menorRank}
\STATE tipoDeEventoParaRealocar $\leftarrow$ tipoDeEventoTempo
\ELSE
\STATE tipoDeEventoParaRealocar $\leftarrow$ tipoDeEventoFluxo
\ENDIF
